 ...
 *
 * Re-running seed resets the admin password to admin123 (fixes invalid login).
 */
...
async function main() {
  // Seed system settings (from original schema.sql)
  ...
  // PHP parity: all system settings from SystemSettingsController defaultSettings
  ...
  // Admin: upsert so re-running seed always resets password (fixes "Invalid email or password")
  ...
  if (!agentRecord) {
    ...
    console.log("Created agent record AG001");
  }

  // Client (user + Client record, assigned to seeded agent)
  ...
  if (!clientRecord) {
    ...
    console.log("Created client record CL001");
