...
export default async function ClientEmergencyWithdrawalPage({
  ...
  const [daysCollected, totalCollectedAgg, existingRequest, display] = await Promise.all([
  ...
  return (
    <>
      ...
      <ModernCard
        ...
        subtitle={isFlexible
          ? `Cycle has ${daysCollected} days collected (total ${formatCurrencyFromGhs(totalCollected, display)}). Commission (${formatCurrencyFromGhs(commissionAmount, display)}) will be deducted.`
          : `Cycle has ${daysCollected} days collected. One day commission (${formatCurrencyFromGhs(commissionAmount, display)}) will be deducted.`}
      >
        <div className="mb-4 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">Available amount: {formatCurrencyFromGhs(availableAmount, display)}</p>
        </div>
