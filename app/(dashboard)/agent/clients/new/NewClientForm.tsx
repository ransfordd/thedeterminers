...
export function NewClientForm() {
  ...
  const dailyAmountRef = useRef<HTMLInputElement | null>(null);
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={() => {
                  ...
                  if (dailyAmountRef.current) dailyAmountRef.current.value = fixedAmountRef.current || "20";
                }}
                className="rounded"
              />
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={() => {
                  setDepositType("flexible_amount");
                  if (dailyAmountRef.current) {
                    fixedAmountRef.current = dailyAmountRef.current.value || fixedAmountRef.current;
                    dailyAmountRef.current.value = "0";
                  }
                }}
                className="rounded"
              />
              <span className="text-sm">Flexible (amount varies)</span>
            </label>
          </div>
        </div>
        <div>
          <label htmlFor="dailyDepositAmount" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Daily Deposit Amount (GHS){depositType === "fixed_amount" ? <span className="text-red-500"> *</span> : null}
          </label>
          <input
          ...
          {depositType === "flexible_amount" ? (
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              Flexible clients don’t have a fixed daily amount; this will be saved as 0.
            </p>
          ) : null}
        </div>
      </div>
