...
export default async function AdminDashboardPage() {
  ...
  const [metrics, recentTransactions, recentApplications, agentPerformance, recentActivities, agentsList, display] =
    await Promise.all([
  ...
  const agentColumns: DataTableColumn<AgentPerformanceRow>[] = [
    ...
    { key: "totalCollections", header: "Total Collections", render: (r) => formatCurrencyFromGhs(r.totalCollections, display) },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard icon={<i className="fas fa-arrow-down text-green-600" />} value={formatCurrencyFromGhs(metrics.totalDeposits, display)} label="Total Deposits" sublabel="All Susu collections" variant="success" />
          <StatCard icon={<i className="fas fa-arrow-up text-amber-600" />} value={formatCurrencyFromGhs(metrics.totalWithdrawals, display)} label="Total Withdrawals" sublabel="Susu payouts" variant="warning" />
          <StatCard icon={<i className="fas fa-chart-pie text-cyan-600" />} value={metrics.systemRevenue != null ? formatCurrencyFromGhs(metrics.systemRevenue, display) : "—"} label="System Revenue" sublabel="Interest + Commission" variant="info" />
